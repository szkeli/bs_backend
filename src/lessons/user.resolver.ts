...
export class UserResolver {
  ...
  @ResolveField(of => LessonsConnection, {
    description: '当前用户的所有课程'
  })
  async lessons (
  @Parent() user: User,
    @Args() args: RelayPagingConfigArgs,
    @Args() filter: FilterLessonArgs
  ) {
  ...
  async lessonsRelayForward (
    id: string,
    first: number,
    after: string | null,
    filter: FilterLessonArgs
  ) {
    const { startYear, endYear, semester } = filter
