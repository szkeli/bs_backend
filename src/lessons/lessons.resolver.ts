...
export class LessonsResolver {
  ...
  @Query(of => LessonNotificationSettings, {
    description: '获取当前用户上课通知的设置'
  })
  @Roles(Role.User)
  ...
  @Mutation(of => LessonNotificationSettings, {
    description: '更新用户上课通知设置'
  })
  @Roles(Role.User)
  ...
  @CurrentUser() user: User,
    ...
    return await this.lessonsService.updateLessonNotificationSettings(
      user.id,
      args
    )
  }
  ...
  async updateLesson (
  @CurrentUser() user: User,
    @Args() args: UpdateLessonArgs
  ) {
    return await this.lessonsService.updateLesson(user, args)
  }
  ...
  @CurrentUser() user: User,
    ...
    return await this.lessonsService.deleteLesson(user, lessonId)
  }

  @Mutation(of => Lesson, { description: '管理员添加课程到某用户' })
  @Roles(Role.Admin)
  async addLessonToUser (
  @CurrentAdmin() admin: Admin,
    @Args() args: AddLessonArgs,
    @Args('to') to: string
  ) {
